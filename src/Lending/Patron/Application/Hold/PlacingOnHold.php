...
final readonly class PlacingOnHold implements LoggerAwareInterface
{
    use LoggerAwareTrait;

    public function __construct(
        public FindAvailableBook $findAvailableBook,
    ...
    public function placeOnHold(PlaceOnHoldCommand $command): TryTo
    {
        return TryTo::run(function () use ($command) {
            ...
            ->onFailure(static fn (Throwable $throwable) => $this->logger?->error('Failed to place a hold', ['exception' => $throwable]))
        ;
    }
    ...
    private function findAvailableBook(BookId $bookId): AvailableBook
    {
        return $this->findAvailableBook
            ...
            ->getOrElseThrow(new InvalidArgumentException(sprintf('Cannot find available book with Id: %s', $bookId->bookId)))
        ;
    }
